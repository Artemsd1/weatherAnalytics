...
day = soup.find("div", class_ = "widget-row widget-row-days-date")
dates = day.find_all("a", class_ = "row-item")
print("Дни:")
days = []
for (i, elem) in enumerate (dates):
    if i == 0 or i == 9:
        days.append(elem.find("div", class_="day").text + ', ' + elem.find("div", class_="date").text)
    else:
        days.append(elem.find("div", class_="day").text + ', ' + elem.find("div", class_="date").text + " дек")
...
for i in table_row_yandex:
    ...
    if day_part.text == "днём":
        ...
        degrees_yandex_day.append(parse_numeric(degrees_temp[0].find("span", class_="temp__value temp__value_with-unit").text))
        wind_yandex_day.append(i.find("span", class_="wind-speed").text)
        pressure_yandex_day.append(i.find("td", class_="weather-table__body-cell weather-table__body-cell_type_air-pressure").text)
        humidity_yandex_day.append(i.find("td", class_="weather-table__body-cell weather-table__body-cell_type_humidity").text.replace('%', ''))
    if day_part.text == "ночью":
        degrees_temp = i.find_all("div", class_="temp")
        degrees_yandex_night.append(parse_numeric(degrees_temp[0].find("span", class_="temp__value temp__value_with-unit").text))
        wind_yandex_night.append(i.find("span", class_="wind-speed").text)
        pressure_yandex_night.append(i.find("td", class_="weather-table__body-cell weather-table__body-cell_type_air-pressure").text)
        humidity_yandex_night.append(i.find("td", class_="weather-table__body-cell weather-table__body-cell_type_humidity").text.replace('%', ''))
...
for i in table_row_mail:
    ...
    if day_part.text == "день":
        degrees_mail_day.append(parse_numeric(i.find("span", class_="text text_block text_bold_medium margin_bottom_10").text.replace('°', '')))
    ...
    if day_part.text == "ночь":
        degrees_mail_night.append(parse_numeric(i.find("span", class_="text text_block text_bold_medium margin_bottom_10").text.replace('°', '')))
        humidity_temp = i.find("span", class_="link link_block link_icon")
...
print("ОБъекты градусы")
print(date_weather[0].degrees)
print(date_weather[1].degrees)

for i, j, m, n in zip(tempr_degrees, wind_speed, pressure_max, humidity_arr):
    yandex.append(i.find("span", class_="unit unit_temperature_c").text)
    yandex.append(j.find("span", class_="wind-unit unit unit_wind_m_s").text)
    yandex.append(m.find("span", class_="unit unit_pressure_mm_hg_atm").text)
    yandex.append(n.text)
...
@app.route('/test')
def test():
    return render_template("test.html", days = days, yandex = yandex)


@app.route('/test2')
def test2():
    return render_template("test2.html", days=days, yandex=yandex)
